...
export async function getPlayerData(regionPrefix: string, summoner: string) {
  ...
  try {
    // prettier-ignore
    const targetIdentity = await fetch(`https://${cluster}.api.riotgames.com/riot/account/v1/accounts/by-riot-id/${summonerName}/${summonerTag}?api_key=${APIKEY}`).then((res) => {if (!res.ok) throw new Error(`FETCH ERROR: ACCOUNT IDENTITY. STATUS: ${res.status}`);return res.json();});
    // prettier-ignore
    const [targetProfile, matchIdList] = await Promise.all([
      ...
      fetch(`https://${cluster}.api.riotgames.com/lol/match/v5/matches/by-puuid/${targetIdentity.puuid}/ids?start=0&count=3&api_key=${APIKEY}`).then((res) => {if (!res.ok) throw new Error(`FETCH ERROR: MATCH ID LIST. STATUS: ${res.status}`);return res.json();}),
    ]);
    // prettier-ignore
